...
import styles from "./rose-afterglow.module.css";
import { SeatingArrangement } from "@/components/invitation/seating-arrangement";
...
const story = [
  {
    date: "October 2025",
    title: "The First Day",
    copy: "Sofia and Samuel first crossed paths at a dear friend’s wedding. It was a fleeting moment, yet it left a mark neither could forget.",
  },
  {
    date: "February 2026",
    title: "The Yes Day",
  ...
  },
  {
    ...
    copy: "Beneath a golden sunset, Samuel asked Sofia to write the next chapter together. She said yes—and here we are.",
  },
];
...
const glimpsePhotos = [
  { src: "/templates/rose-afterglow/rose-afterglow.webp", alt: "An enchanted rose garden glowing at sunset" },
  { src: "/templates/rose-afterglow/couple-hands.webp", alt: "Bride and groom reaching for one another" },
  { src: "/templates/rose-afterglow/scratch/hero-olive.webp", alt: "Bride and groom within the olive wedding portrait artwork" },
  { src: "/templates/rose-afterglow/scratch/hero-wide-olive.webp", alt: "A wide olive wedding portrait scene" },
  { src: "/templates/rose-afterglow/scratch/hero.webp", alt: "A romantic wedding portrait in warm tones" },
  { src: "/templates/rose-afterglow/scratch/hero-wide.webp", alt: "A wide romantic wedding portrait" },
  { src: "/templates/rose-afterglow/curtains/olive.webp", alt: "Olive wedding curtains framed by flowers and greenery" },
  { src: "/templates/rose-afterglow/curtains/default.webp", alt: "Wedding curtains surrounded by soft floral details" },
  { src: "/templates/rose-afterglow/moonlit-bloom.webp", alt: "A moonlit floral wedding scene" },
  { src: "/templates/coastal-reverie/coastal-reverie.webp", alt: "A soft romantic coastal wedding scene" },
];

const heroCouplePhoto = {
  src: "/templates/rose-afterglow/couple-hands.webp",
...
export function RoseAfterglowInvitation() {
  ...
  const invitationUnlocked = rosePhase === "revealed";
  ...
  useEffect(() => {
    if (rosePhase !== "celebrating") return;
    const reducedMotion = window.matchMedia("(prefers-reduced-motion: reduce)").matches;
  ...
  const replayOpening = () => {
    ...
    setRosePhase("curtains");
  };

  return (
    <main className={`${styles.invitation} ${styles.roseVariant} ${invitationUnlocked ? styles.roseUnlocked : ""} ${rosePhase !== "curtains" ? styles.invitationReady : ""}`} id="invitation-top">
      <div className={`${styles.roseCurtainIntro} ${rosePhase !== "curtains" ? styles.roseCurtainsOpening : ""} ${rosePhase !== "curtains" && rosePhase !== "opening" ? styles.roseCurtainsGone : ""}`} aria-hidden={rosePhase !== "curtains" && rosePhase !== "opening"}>
          <div className={`${styles.roseCurtainPanel} ${styles.roseCurtainLeft}`} aria-hidden="true"><img src="/templates/rose-afterglow/curtains/olive.webp" alt="" /></div>
          <div className={`${styles.roseCurtainPanel} ${styles.roseCurtainRight}`} aria-hidden="true"><img src="/templates/rose-afterglow/curtains/olive.webp" alt="" /></div>
      ...
      <a className={styles.weddingDetailsButton} href="#event-details" aria-hidden={!invitationUnlocked} tabIndex={invitationUnlocked ? 0 : -1}><CalendarDays aria-hidden="true" /> Wedding details</a>

      <section className={styles.countdownSection} id="counting" aria-labelledby="countdown-title">
        <div className={`${styles.gardenPortal} ${styles.portalLeft}`} data-wedding-reveal aria-hidden="true"><img src="/templates/rose-afterglow/rose-afterglow.webp" alt="" /></div>
        <div className={`${styles.gardenPortal} ${styles.portalRight}`} data-wedding-reveal aria-hidden="true"><img src="/templates/rose-afterglow/rose-afterglow.webp" alt="" /></div>
      ...
      <section className={styles.glimpseSection} aria-labelledby="glimpse-title">
      ...
      <section className={styles.storySection} aria-labelledby="story-title">
        <div className={`${styles.storyBloom} ${styles.storyBloomLeft}`} data-wedding-reveal aria-hidden="true"><img src="/templates/rose-afterglow/rose-afterglow.webp" alt="" /></div>
        <div className={`${styles.storyBloom} ${styles.storyBloomRight}`} data-wedding-reveal aria-hidden="true"><img src="/templates/rose-afterglow/rose-afterglow.webp" alt="" /></div>
      ...
      <section className={styles.memorySection} aria-labelledby="memory-title">
        <div className={styles.memoryBackdrop} aria-hidden="true"><img src="/templates/rose-afterglow/rose-afterglow.webp" alt="" /></div>
      ...
      <section className={styles.eventsSection} id="event-details" aria-labelledby="events-title">
  ...
  const scratchAt = (event: ReactPointerEvent<HTMLCanvasElement>) => {
    ...
    for (let gx = 0; gx < columns; gx += 1) {
      for (let gy = 0; gy < rows; gy += 1) {
        const normalizedX = ((gx + 0.5) / columns - 0.5) * 2;
        const normalizedY = ((gy + 0.5) / rows - 0.5) * 2;
    ...
    <section className={`${styles.scratchHero} ${revealed ? styles.scratchHeroRevealed : ""}`} aria-labelledby="rose-couple-names">
      ...
      <div className={styles.scratchHeroArtwork}>
        <div className={styles.scratchPhotoComposition}>
          ...
          <img className={styles.scratchOrnateFrame} src="/templates/rose-afterglow/frame-ivory.webp" alt="" aria-hidden="true" />
